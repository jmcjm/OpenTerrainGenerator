...
- **Destination portals carry the return color** (port of the 1.20.1 fix). The portal auto-created on the destination side exists for the return trip, so it now carries the color of the level the entity is leaving instead of the travel color — travelling back finds the portal the player originally came from. A portal of the dimension's own color (or the overworld's color) leads back to the overworld; any other color travels to the dimension that color belongs to (cross-dimension portals, OTG and non-OTG alike). A portal of the overworld preset's own color standing in the overworld deliberately leads nowhere — entering one used to spawn a second dimension of the overworld's preset. Auto-created frames use the PortalBlocks of the preset/dimension the color belongs to, regardless of where the portal stands. Return-trip color matching now also goes through the deduplicated PortalTarget colors, fixing a mismatch when two presets collided on the same raw color.
- **Fixed: returning through a portal often spawned a fresh portal next to the original** (port of the 1.20.1 fix). The teleporter searched for existing portals with a sparse block scan (every 4th column) that routinely misses 1-block-thick portal planes. Portals are now tracked in `SharedPortalRegistry` — ignition and the teleporter both register positions per (dimension, color), persisted in `otg_world_data.json` — and the teleporter checks the registry first (validating the portal still stands), falling back to the scan only for unregistered portals. Registered portals only link within 128 horizontal blocks of the scaled coordinates, like vanilla — a fresh portal far away creates a new arrival portal instead of linking across the world. Registry entries are dropped when a dimension is deleted. Also documented: per-dimension `DoDaylightCycle`/`DoWeatherCycle` cannot freeze time/weather outside the overworld — MC stores both once per save, driven by the overworld.
- **Fixed: an invalid `PortalColor` produced a portal that teleported nowhere.** Colors outside the portal block palette (`PortalColors.COLORS`, e.g. a typo like "purple") passed through unvalidated; the block lookup silently fell back to the `default` block, whose color no longer matched the portal target's — the portal ignited but never matched a destination. Unknown colors are now coerced to `default` with a warning listing the valid palette.
...
- **Non-OTG dimensions in WorldPreset YAML** — `NonOTGWorldType` now works on `Nether:`, `End:` and `Dimensions:` entries (previously overworld-only). Any registered MC WorldPreset can be referenced (`minecraft:flat`, `minecraft:amplified`, or a modded world type by `modid:name`) — its main (overworld) generator is mounted in the slot, so `flat` as the Nether means a flat nether, consistent with how OTG presets behave in slots. This lets overworld-replacing worldgen mods run as separate dimensions too — a mod that only injects biomes into the vanilla overworld (Biomes O' Plenty, via TerraBlender) is mounted as `minecraft:normal` and its biomes come along; a mod that registers its own world type is referenced by that key. Non-OTG `Dimensions:` entries require a `DimensionName` (registered as `otg:<name>`) and support OTG portals (`PortalBlocks`/`PortalColor`/`PortalIgnitionSource`, YAML-only config) and per-dimension GameRules (applied over vanilla defaults). Invalid entries (both `PresetFolderName` and `NonOTGWorldType`, or missing `DimensionName`) are pruned with a log. `Seed` on non-OTG entries is ignored (world seed applies). Editor shows non-OTG entries read-only.
- **Documented: several GameRules are overworld-scoped and ignore a per-dimension override.** Beyond `DoDaylightCycle`/`DoWeatherCycle` (time/weather stored once per save), Minecraft reads `DoPatrolSpawning`/`DoTraderSpawning`/`DoInsomnia` (their spawners only tick on the overworld), `SpawnRadius`/`SpawnChunkRadius` (world spawn lives in the overworld), `MaxCommandChainLength`/`MaxCommandForkCount`/`LogAdminCommands` (read from the server's overworld rules) and `PlayersSleepingPercentage` (only the overworld allows sleeping) from the overworld regardless of which dimension the rule is set on. The WorldPreset docs now list these; every other rule honours a per-entry override.
