...
public class BoulderResource extends FrequencyResourceBase
{
    ...
    public BoulderResource(BiomeSettings config, List<String> args) throws InvalidConfigException
    {
        super(config, args);
        assureSize(6, args);
    ...
    public void spawn(IWorldGenRegion worldGenRegion, Random random, int x, int z)
    {
        ...
        // TODO: It seems that once, 'i' controlled the jitter and size of the boulder - should consider
        //  reimplementing this as a configurable feature; we can give it both an initial value, and a range of
        //  values to increase/decrease by per loop as a jitter.
        while ((i >= 0) && (j < 3)) {
