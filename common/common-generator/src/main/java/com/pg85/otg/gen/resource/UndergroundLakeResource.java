...
public class UndergroundLakeResource extends FrequencyResourceBase
{
    ...
    public void spawn(IWorldGenRegion worldGenRegion, Random rand, int x, int z)
    {
        ...
        for (int i = 0; i <= size; i++) {
            ...
            for (
                int xLake = (int) (xAdjusted - horizontalSize / 2.0D);
                xLake <= (int) (xAdjusted + horizontalSize / 2.0D);
                xLake++
            ) {
                for (
                    int yLake = (int) (yAdjusted - verticalSize / 2.0D);
                    yLake <= (int) (yAdjusted + verticalSize / 2.0D);
                    yLake++
                ) {
                    for (
                        int zLake = (int) (zAdjusted - horizontalSize / 2.0D);
                        zLake <= (int) (zAdjusted + horizontalSize / 2.0D);
                        zLake++
                    ) {
